...
lint:
	flake8 sceptre_template_fetcher tests integration-tests
...
coverage-ci:
	coverage erase
	coverage run --source sceptre_template_fetcher -m pytest
	coverage html
...
docs:
	rm -f docs/sceptre_template_fetcher.rst
...
docs-api:
	rm -f docs/_api/sceptre_template_fetcher.rst
	rm -f docs/_api/modules.rst
	sphinx-apidoc -o docs/_api sceptre_template_fetcher
